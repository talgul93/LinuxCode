...
int main(void)
{
	...
	//파일 생성과 오픈
	fd = open("./copy.txt", O_WRONLY|O_CREAT|O_TRUNC, 0644);
	if(fd == -1)
	{
		perror("open");
		exit(EXIT_FAILURE);
	...
	//공유메모리의 내용을 읽어 파일에 저장
	bytecnt = write(fd, (shmaddr+sizeof(int)*2), SHMSIZE);
	
	/*
	while(1)
	{
		printf("자식: while: total = %d\n",*(shmaddr));
		if(*(shmaddr) == 0)
			break;
		bytecnt = write(fd, (shmaddr+sizeof(int)),SHMSIZE);
		if(bytecnt == -1)
		{
			perror("write");
			exit(EXIT_FAILURE);
		}
		*(shmaddr) = *(shmaddr)-bytecnt;
		sleep(2);
	}
	...
	*/
	...
	printf("자식 : flag = %d, total = %d\n",*(shmaddr),*(shmaddr+sizeof(int)));
