//아스키아트 소스파일을 스레드를 생성해서 구현하도록 바꿔보자!!
...
int main()
{
	...
	pthread_t pthread;
	int err;
	struct DATA *data4thr;	//쓰레드 인자로 전달할 값
	data4thr = (struct DATA*) malloc (sizeof(struct DATA));
	...
	data4thr->size = size;
	data4thr->width = width;
	data4thr->height = height;
	...
	err = pthread_create(&pthread, NULL, write_thr, data4thr);
	if(err!=0)
	{
...
void* write_thr (void *data)
{
	...
	{
		free(temp->image);      // 픽셀 데이터를 저장한 동적 메모리 해제
		return 0;         // 프로그램 종료
	}
	...
	{   
		...
		for (int x = 0; x < temp->width; x++)
		{
			...
			int index = (x * PIXEL_SIZE) + (y * (temp->width * PIXEL_SIZE)) + (temp->padding * y); 

			// 현재 픽셀의 주소를 RGBTRIPLE 포인터로 변환하여 RGBTRIPLE 포인터에 저장
			RGBTRIPLE *pixel = (RGBTRIPLE *)&temp->image[index];
